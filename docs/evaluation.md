...
Untouched frames are **not** negative labels. Decisions recorded by
`fotosort decisions import` copy a report's own `selected` column, so they are
skipped (and counted in the command's output) unless `--include-imported` is
given; otherwise a baseline would score perfectly against its own picks. Stale
feedback must be cleared or reviewed again before export. The command refuses
...
A legacy absolute path matches a declared photo only when exactly one declared
`file` ends that path; the evaluator does not guess an old root, and an
ambiguous path is an error. Duplicate IDs/paths, extra report photos, malformed
values, and contradictory keep/reject labels are rejected. The summary `--output`
is checked before evaluation and can never be the manifest, a report, a photo,
or a review record, under any spelling of that file.
...
A comparison stops when two recorded hashes for the same photo differ. A photo
without a hash on either side is unverified, not different, and each comparison
entry states `content_identities_verified`.
...
including when other exposures are unknown: the known count is a lower bound.
`budget_compliance.candidates` is `true` only when every exposure is known.
...
`recall`, `recall_lower_bound`, and `recall_upper_bound`. `recall` is a number
only when nothing is unknown: a group with an unknown member can turn into a hit
but never into a miss, so a ratio over the known groups alone would be biased
upward. With unknown groups `recall` is `null` and the bounds state what is
established, from every unknown group missed to every unknown group hit. An
empty denominator produces `null`, not zero. Split aggregates sum the counts
...
| `pairwise` | Unique pairs split into preferred-only, other-only, both, neither, and unknown outcomes; the latest preference for a pair replaces an earlier or reversed one |
...
it does not by itself make a selected/omitted result unknown. Analysis without
a judge writes `shortlisted=0` on every row, which records no exposure at all:
when a report shortlists nothing, every exposure is unknown and shortlist
recall is `null`. Acceptable groups listed twice are counted once.
